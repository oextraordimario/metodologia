...
* Requires unique old\_string or use replace\_all parameter
* Use replace\_all for renaming variables throughout a file
* Fails if old\_string is not unique (unless replace\_all is true)
...
* Output modes: content (lines), files\_with\_matches (paths), count (counts)
* Supports context lines (-A, -B, -C)
* Case insensitive option (-i)
* Multiline matching support
* Can limit results with head\_limit and offset
...
* Can run in background with run\_in\_background parameter
...
* Takes bash\_id to identify the shell
...
* Task states: pending, in\_progress, completed
* Must provide both content and activeForm for tasks
* Keep exactly ONE task in\_progress at a time
...
* Only use skills listed in available\_skills
